...
const StyledHeightDisplay = styled(Box)(({ theme }) => ({
  display: 'flex',
  alignItems: 'baseline',
  gap: theme.spacing(1),
...
interface HeightRecordCardProps {
  id: string;
  height: number;
  date: Date;
  notes?: string;
...
/**
 * Card component for displaying height record entries in a list.
 * Shows height with unit conversion, date, and actions.
 *
 * Features:
 * - Displays height in both cm and feet/inches
 * - Shows formatted date
 * - Provides action menu for view, edit, delete
 * - Handles optional notes display
 * - Hover animations and semantic data-testids
 */
export const HeightRecordCard: React.FC<HeightRecordCardProps> = ({
  id,
  height,
  date,
  notes,
  ...
  const heightInFeetInches = `${feet}'${inches}"`;
        ...
        <StyledCardContent>
          ...
          >
            <Typography variant='h6' component='h3' sx={{ fontWeight: 600 }}>
              {t('bodyMetrics.heightRecord')}
            </Typography>
            <Icon name='Height' sx={{ fontSize: '1.5rem', color: 'primary.main' }} />
          </Box>
          ...
          <StyledHeightDisplay>
            <Typography
              ...
              data-testid={`${dataTestId}-height-cm`}
            >
              {height}
            </Typography>
            ...
            <StyledConversionChip
              label={heightInFeetInches}
              variant='outlined'
              size='small'
              data-testid={`${dataTestId}-height-feet-inches`}
            />
          </StyledHeightDisplay>
