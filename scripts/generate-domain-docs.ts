...
/**
 * Generates comprehensive PUBLIC API documentation for the Domain Layer.
 * 
 * This script scans all domain classes and extracts:
 ...
 */
async function generateDomainLayerDocs(): Promise<void> {
  const project = new Project({
    tsConfigFilePath: path.join(process.cwd(), 'tsconfig.json'),
  });
  
  project.addSourceFilesAtPaths('src/features/**/domain/**/*.ts');
  project.addSourceFilesAtPaths('src/shared/domain/**/*.ts');

  const sourceFiles = project.getSourceFiles();
  let markdown = generateHeader();
  
  // Generate class inventory first
  markdown += generateClassInventory(sourceFiles);

  console.log(`Found ${sourceFiles.length} source files in Domain Layer`);
  ...
  fs.writeFileSync('PUBLIC_API_DOMAIN.md', markdown);
  console.log('✅ Domain Layer PUBLIC API documentation generated successfully at PUBLIC_API_DOMAIN.md');
}
...
function generateHeader(): string {
  return `# PUBLIC API - Domain Layer

This document provides comprehensive **PUBLIC API** documentation for the Domain Layer.
Every class, method, input parameter and output value is documented with complete signatures and descriptions.

## PUBLIC API Overview

The Domain Layer exposes the following PUBLIC API:
- **Pure Business Logic**: Core business entities and value objects
- **Immutable Value Objects**: Validated, immutable data structures
- **Rich Domain Models**: Entities containing business behavior
- **Domain Events**: Event system for cross-aggregate communication
- **Invariant Protection**: Business rule enforcement
...
/**
 * Generates a complete inventory of all classes and their public methods
 */
function generateClassInventory(sourceFiles: any[]): string {
  let inventory = `## PUBLIC API Class Inventory\n\n`;
  inventory += `This section provides a complete index of all PUBLIC API classes and methods.\n\n`;
  
  const allClasses: Array<{className: string, filePath: string, methods: string[]}> = [];
  ...
  for (const sourceFile of sourceFiles) {
    ...
    for (const classDec of exportedClasses) {
      ...
      const publicMethods = classDec
        .getInstanceMethods()
        .filter((m: any) => m.getScope() === Scope.Public)
        .map((m: any) => m.getName());
        
      const staticMethods = classDec
        .getStaticMethods()
        .filter((m: any) => m.getScope() === Scope.Public)
        .map((m: any) => `static ${m.getName()}`);
        
      const allMethods = [...publicMethods, ...staticMethods];
  ...
  inventory += `### Classes and Methods Overview\n\n`;
  inventory += `| Class | File | Public Methods |\n`;
  inventory += `|-------|------|----------------|\n`;
  
  for (const classInfo of allClasses) {
    const methodsList = classInfo.methods.map(method => `\`${classInfo.className}.${method}()\``).join(', ');
    inventory += `| **${classInfo.className}** | \`${classInfo.filePath}\` | ${methodsList} |\n`;
  }
...
// Execute the script
generateDomainLayerDocs().catch((error) => {
  console.error('❌ Error generating Domain Layer documentation:', error);
  process.exit(1);
});
